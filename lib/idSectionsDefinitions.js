...
class IdSectionDefinitionBase {
	...
	clear(guardstore) {
		return guardstore.clear(this.name);
	}
...
class Base32IdSectionDefinition extends IdSectionDefinitionBase {
	...
	getRegex() {
		return `[${base32.regexCharters}]{${this.size}}`;
	}
	...
	async getPrevious({ date, value }) {
		if (!date || !vlaue)
			throw new Error('No date or value supplied');
		const date32 = base32.fromDate(new Date(date));
		return {
			value,
			date32,
		};
